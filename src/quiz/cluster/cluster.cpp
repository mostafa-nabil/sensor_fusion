...
void proximity(std::vector<std::pair<std::vector<float>,bool>> &pointsAug, KdTree* &tree, float distanceTol, int &id,std::vector<int> &cluster)
{
	pointsAug[id].second = true;
	cluster.push_back(id);
	std::vector<int> nearby = tree->search(pointsAug[id].first,distanceTol);
	
	for(int i = 0; i<nearby.size(); i++)
	{
		if 	(pointsAug[nearby[i]].second == false){
			proximity(pointsAug,tree,distanceTol,nearby[i],cluster);
		}
	}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	std::vector<std::pair<std::vector<float>,bool>> pointsAug;
	for(int i=0 ; i<points.size(); i++)
	{
		std::pair<std::vector<float>,bool> pointAug = {points[i],false};
		pointsAug.push_back(pointAug);
	}

	for (int i = 0; i < points.size(); i++)
	{
		if(pointsAug[i].second == true)
		{
			continue;
		}
		std::vector<int> cluster;
		proximity(pointsAug,tree,distanceTol,i,cluster);
